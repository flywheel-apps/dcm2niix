"""Utility functions for dcm2niix-pydeface gear."""
...
def remove_incomplete_volumes(dcm2niix_input_dir):
    ...
    log.info(f'Running incomplete volume correction. {n_dicom_files} dicom files found.')
    ...
    if process.wait() != 0:
        ...
        if os.path.isdir(corrected_dcm_dir):
            ...
            if len(subdirs) != 2:
                log.error(('Output from incomplete volume removal script is unexpected. '
                           'Exiting.'))
                os.sys.exit(1)
            ...
            log.info(('No file removal performed. '
                      'No dicoms from incomplete volumes were found.'))

        n_dicom_files = len(glob.glob(dcm2niix_input_dir + '/*'))
        log.info((f'{n_dicom_files} dicom files remain. '
...
def decompress_dicoms(dcm2niix_input_dir):
    ...
    log.info(f'Running decompression of dicom files. {n_dicom_files} dicom files found.')
...
def coil_combine(nifti_files):
    ...
    Args:
        nifti_files (list): list of nifti files to generate coil combined data for
    ...
    for nifti_file in nifti_files:
        ...
        try:
            ...
            log.info(f'Starting to generate coil combined data for {nifti_file}')
            ...
            log.error(f'Could not generate coil combined data for {nifti_file}. Exiting.')
            log.exception(e)
            os.sys.exit(1)
