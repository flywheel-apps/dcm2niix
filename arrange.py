"""Functions to arrange gear inputs."""
...
def prepare_dcm2niix_input(infile, rec_infile, work_dir):
    ...
    Args:
        ...
        rec_infile (str): the absolute path to the rec file for a par/rec file pair;
            note, the infile must be a valid par file.
    ...
    if infile.endswith('.zip'):
        ...
        try:
            ...
            log.exception(('Incorrect gear input. '
        ...
        try:
            ...
            log.exception((f'Incorrect gear input. '
        ...
        if infile.lower().endswith('par'):
            ...
            try:
                ...
                log.exception((f'Incorrect gear input. If rec_file_input provided, '
        ...
        log.error(f'Incorrect gear input. Exiting.')
        os.sys.exit(1)

    log.info('dcm2niix input prepared successfully.')
...
def is_archive_empty(archiveObj):
    ...
    if type(archiveObj) == zipfile.ZipFile:
        ...
        log.info('Unsupported archive format. Unable to establish size of input archive.')
...
def extract_archive_contents(archiveObj, work_dir):
    ...
    # 1. set the stage for zip or tar archive
    if type(archiveObj) == zipfile.ZipFile:
    ...
    if len(subdirs) == 0:
        ...
        if type(archiveObj) == zipfile.ZipFile:
            archiveObj.extractall(dcm2niix_input_dir,
                                  strip_prefix_ziparchive(archiveObj, subdirs[0]))

        if type(archiveObj) == tarfile.TarFile:
            archiveObj.extractall(dcm2niix_input_dir,
                                  strip_prefix_tararchive(archiveObj, subdirs[0]))

    else:

        # multiple subdirectories, input packaging not supported, exit
        log.error((f'Incorrect gear input. Input archive packaging is not supported. '
                    'Multiple subdirectories detected in archive: {subdirs}. Exiting.'))
        os.sys.exit(1)
...
def strip_prefix_ziparchive(zipObj, prefix):
    """Generator to modify a filepath in a zip archive without the specified prefix."""

    for zipinfo in zipObj.infolist():

        if (zipinfo.filename.startswith(prefix) and
            os.path.splitext(zipinfo.filename)[0] != prefix):
...
def strip_prefix_tararchive(tarObj, prefix):
    """Generator to modify a filepath in a tar archive without the specified prefix."""

    for tarinfo in tarObj.getmembers():

        if (tarinfo.path.startswith(prefix) and
            os.path.splitext(tarinfo.path)[0] != prefix):
...
def adjust_parrec_filenames(dcm2niix_input_dir, filename):
    ...
    files = [file for file in files if file.lower().endswith('.par')
                                    or file.lower().endswith('.rec')]
